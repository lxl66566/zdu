use std::{ffi::OsStr, io, process::Output, str, sync::Once};
...
#[allow(clippy::unnecessary_wraps)] // only the unix branch can fail
...
fn run_cmd<T: AsRef<OsStr>>(command_args: &[T]) -> Output {
    initialize();
    let mut to_run = cargo_bin_cmd!("zdu");
    // Hide progress bar
    to_run.arg("-P");
...
fn exact_stdout_test<T: AsRef<OsStr>>(command_args: &[T], valid_stdout: &[String]) {
...
fn exact_stderr_test<T: AsRef<OsStr>>(command_args: &[T], valid_stderr: &str) {
    let to_run = run_cmd(command_args);
...
// "windows" result data can vary by host (size seems to be variable by one byte); fix code vs test
// and re-enable
